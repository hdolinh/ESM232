...
# first as a for loop for illustration
# use as numeric to extract parameters
res = rep(0, times=nsim)
for (i in 1:nsim) {
  tmp = evolve_pop(
    fertility=as.numeric(sens_micro$X[i,c("f1","f2","f3")]), 
    survivability=as.numeric(sens_micro$X[i,c("p12","p23","p34")]), 
    initialpop=ini, nstep=nmonths)
  res[i] = tmp$poptot[12]
}

# ALTERNATIVE (faster) implement with apply
...
p_wrapper = function(x, func, initialpop, nstep ) {
fertility=as.numeric(c(x["f1"], x["f2"], x["f3"]))
survivability= as.numeric(c( x["p12"], x["p23"], x["p34"]))
res = evolve_pop(survivability =survivability, fertility = fertility, initialpop=initialpop, nstep=nstep)
# now return the final population total
return(finalpop=res$poptot[nstep])
}

# use apply here so we can specify rows of our sensitivity analysis parameter object 
res = apply(sens_micro$X,1, p_wrapper,  initialpop=ini, nstep=nmonths, func=evolve_pop)
