...
p2 = ggplot(as.data.frame(res), aes(pred, prey, col=as.factor(round(time/10))))+geom_point()
p2

grid.arrange(p1,p2)

# try with different parameter sets, can you create one where populations are stable
...
allresults = mapply(FUN=p_wrapper, rprey=sens_pars$rprey, K=sens_pars$K, alpha=sens_pars$alpha, 
                    eff=sens_pars$eff, pmort=sens_pars$pmort,
 MoreArgs=list(currpop=currpop, days=days, func=lotvmodK))

# take results back to unlisted form
allres = as.data.frame(matrix(unlist(allresults), ncol=4, byrow=T))
colnames(allres) = c("maxprey","minprey","maxpred","minpred")
...
ressimple = as.data.frame(res) %>% gather(key="animal", value="pop",-time)
# graph both populations over time
p2=ggplot(ressimple, aes(time, pop, col=animal))+geom_line()
p2

# of course in this case these are not very realistic populations
```
And just for fun
