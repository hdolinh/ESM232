...
#' @return
#' almond yield (anomoly from California mean in ton/acre, and maximum and mininum yields
#'
#'
...
Jan_P = (tmp %>% subset(month==2))$precip
...
yield =
  ...
  maxyield= max(yield)
  minyield=min(yield)
return(list(max=maxyield, min=minyield))
}
